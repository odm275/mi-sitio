...
Candelaris and its businesses are expanding so their needs are in constant flux as they are in the
...
The performance drain would be palpable 🐢. Hence, a good candidates for react hook form.
...
The vast bulk of the state in the app is server state, hence it is handled by React Query, and form state is handled by react hook form. However,
...
The challenge here was to collect user information through different pages. Hence,
we needed a state management solution to store user input through different pages.

I needed a state management solution that would helped me map my checkout flow into states, and save input through different pages.
...
I narrowed down my options down to [useReducer](https://beta.reactjs.org/apis/react/useReducer) and [XState](https://xstate.js.org/).
Libraries like Jotai, and Recoil looked cool, but rather than provider a flux(store, actions, and reducer) like pattern,
they provide better primitives state primitives. Hence, I felt like they didn't fit my use case the best. In my case, I needed to build a "machine".

useReducer in this case is representing the bulk of Flux like state management solutions like Redux and Zustand. That is not to say each of these
state management libraries do not have trade offs of their own between each other. For instance, Zustand would without a doubt have better performance than
useReducer + Context, but given that the client will only submit orders a couple of times a day -- there won't be a performance problem.
impact this use case, their tradeoffs were negligible.
